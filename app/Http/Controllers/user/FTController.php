...
class FTController extends Controller
{
    ...
    function mesFormations(){
        ...
        ->get(['formationtickets.*', 'formations.titre', 'formations.prix', 'formations.imageForm','formations.firstDate']);
    ...
    function DownTicket(Request $req){
        ...
        $info = DB::table('formationtickets')
            ...
            ->get(['formationtickets.idTransaction', 'formationtickets.dateTicket', 'nomSociete', 'nom', 'prenoms', 'email', 'typeActor', 'formations.titre', 'formations.confName', 'formations.lieu', 'formations.firstDate']);

            $chemin= 'codes-qr/'.$req['trans'].'.svg';
            $qrcode = QrCode::size(300)->generate($req['trans'], '../public/'.$chemin) ;

        // L'instance PDF avec la vue resources/views/posts/show.blade.php
        $pdf = PDF::loadHTML(View::make('component.ticketFormation', ['infos'=>$info , 'chemin'=>$chemin ] ));
